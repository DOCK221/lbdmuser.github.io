...
def create_presentation():
    ...
    # Chemins des images (seulement les créations graphiques, pas les bureaux)
    ...
    def add_visual_examples_slide(slide_number):
        """Slide spéciale : 3 images horizontalement avec titres"""
        slide = prs.slides.add_slide(prs.slide_layouts[6])
        ...
        # 3 images horizontalement (sans titres)
        
        # IMAGE 1 - Offres de domiciliation (gauche)
        if os.path.exists(IMG_OFFRES):
            slide.shapes.add_picture(IMG_OFFRES, Inches(0.75), Inches(2.2), width=Inches(2.9))
        
        # IMAGE 2 - Services (centre)
        if os.path.exists(IMG_SERVICES):
            slide.shapes.add_picture(IMG_SERVICES, Inches(3.8), Inches(2.2), width=Inches(2.9))
        
        # IMAGE 3 - Bureau 406 (droite)
        if os.path.exists(IMG_BUREAU406):
            slide.shapes.add_picture(IMG_BUREAU406, Inches(6.85), Inches(2.2), width=Inches(2.9))
    ...
    # SLIDE 23 - EXEMPLES DE CRÉATIONS VISUELLES (3 images horizontalement)
    ...
    # Sauvegarde
    prs.save('/workspace/Soutenance_Mouhamadou_Sow_VERSION_FINALE.pptx')
    print("✅ Présentation VERSION FINALE créée !")
    print("📸 Slide 23 : 3 images horizontalement (créations graphiques uniquement)")
    print("📄 Slides 4 et 6 : textes sans images")
